...
exports.signup = async (req, res) => {
  try {
    const { fullName, username, email, password, confirmPassword, number, gender } = req.body;
...
exports.sendOtp = async (req, res) => {
  try {
    ...
    if (!email) {
      return res.status(400).json({ error: 'Email is required in body or query' });
    }
    ...
    console.log(`Storing OTP for ${email}: ${otp} (expires at ${new Date(expires).toISOString()})`);
